...
object TimeUsage {
  ...
  def dfSchema(columnNames: List[String]): StructType =
    StructType(StructField(columnNames.head, StringType, nullable = false) :: columnNames.tail.map(x => StructField(x, DoubleType, nullable = false)))
  ...
  def classifiedColumns(columnNames: List[String]): (List[Column], List[Column], List[Column]) = {
    ...
    def classifiedColumns_Acc(columnNames: List[String], acc: (List[Column], List[Column], List[Column])
                             ): (List[Column], List[Column], List[Column]) ={
      if (columnNames.isEmpty)
        acc
      else if (primary_needs.exists(columnNames.head.startsWith))
        classifiedColumns_Acc(columnNames.tail, (acc._1 :+ col(columnNames.head), acc._2, acc._3))
      else if (working_activities.exists(columnNames.head.startsWith))
        classifiedColumns_Acc(columnNames.tail, (acc._1, acc._2 :+ col(columnNames.head), acc._3))
      else if (leisure.exists(columnNames.head.startsWith))
        classifiedColumns_Acc(columnNames.tail, (acc._1, acc._2, acc._3 :+ col(columnNames.head)))
      else
        classifiedColumns_Acc(columnNames.tail, acc)
    }
    classifiedColumns_Acc(columnNames, (List[Column](), List[Column](), List[Column]()))
  }
  ...
  def timeUsageSummary(
    ...
    val workingStatusProjection: Column = when((col("telfs") >= 1) && (col("telfs") < 3),
  ...
  def timeUsageGrouped(summed: DataFrame): DataFrame = {
    summed.groupBy("working", "sex", "age").agg(
      ...
      round(mean("other"), 1).as("other")).orderBy("working", "sex", "age")
  }
  ...
  def timeUsageGroupedTyped(summed: Dataset[TimeUsageRow]): Dataset[TimeUsageRow] = {
    ...
    summed.groupByKey(p => (p.working, p.sex, p.age)).agg(
      ...
      round(typed.avg[TimeUsageRow](_.other), 1).as("other").as[Double]).map(r => TimeUsageRow(
      r._1._1, r._1._2, r._1._3, r._2, r._3, r._4)).orderBy("working", "sex", "age")
  }
}
